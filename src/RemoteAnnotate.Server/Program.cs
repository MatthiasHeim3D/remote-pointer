...
using RemoteAnnotate.Server.Health;
...
using PointerSessionOptions = RemoteAnnotate.Server.Sessions.SessionOptions;
...
builder.Services
    ...
    .AddJsonProtocol(options => RemoteAnnotateJson.Configure(options.PayloadSerializerOptions));
builder.Services
    .AddHealthChecks()
            ...
            async (context, next) =>
            {
                if (!context.Request.IsHttps)
                {
                    var logger = context.RequestServices
                        ...
                        .CreateLogger("RemoteAnnotate.Server.TransportSecurity");
                    logger.LogWarning(
