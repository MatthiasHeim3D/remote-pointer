...
public static class AnnotationColors
{
    ...
    /// <summary>
    /// The colours offered as presets and the only ones the relay hands out when it has to move
    /// an annotator off a colour another annotator already holds. They are spaced around the
    /// wheel so neighbours stay tellable apart, and each reads against both a light and a dark
    /// screen, since the shared video underneath could be either. Order is part of the contract:
    /// the relay allocates from it, so changing it changes who gets what.
    /// </summary>
