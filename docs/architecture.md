...
Receiver discovery is controlled by the operator through `Sessions:ReceiverDiscoveryEnabled`. The relay image enables it unless the deployment overrides it, and the shipped Compose environment file turns it off. Receivers still opt in per active session. Direct requests enter the same pending-presenter state as pairing-code requests, so explicit receiver approval remains the sole credential-issuance boundary.
