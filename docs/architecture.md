...
The receiver view model exposes an **Available**/**Invisible** selector. Choosing **Available** for the first time creates and automatically publishes the receiver; later changes update the stored visibility preference. It exposes the pending machine name for explicit approval and locks monitor selection while the receiver presence exists. Incoming events are validated against TTL again immediately before display. Acknowledgements are sent only when the overlay accepts the marker.
