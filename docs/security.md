...
- Changing the password takes effect on the relay immediately rather than at the next hub call, and a published receiver moves to the new group with the connection that published it. The password it left can no longer list or reach it, and a request that no longer shares its group is cancelled before it can be approved. A presenter already approved keeps its connection, which the receiver ends with **Disconnect all senders**.
