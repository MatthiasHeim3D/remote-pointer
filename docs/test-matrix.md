...
1. Run `dotnet run --project src/RemoteAnnotate.Client`.
...
3. Confirm the corresponding protected role file under `%LocalAppData%\RemoteAnnotate\Sessions` is removed on normal shutdown.
4. Inspect `%LocalAppData%\RemoteAnnotate\Logs\audit-YYYYMMDD.jsonl`. Confirm records are valid JSON and contain no coordinates, session/reconnect tokens, exception messages, screen metadata, or typed data.
