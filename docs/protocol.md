...
- `RelayCapabilities`: whether the relay is protected by a server password. A client only ever reads `false` here, since an unprotected relay is the only kind it can reach without one.
...
The server password is presented on the connection rather than through a hub method. The client derives a key from it with PBKDF2-SHA256 — never the password itself — and sends that key as an `Authorization: Bearer` header, so it stays out of proxy access logs. The relay derives the same key from `Access:ServerPassword` at startup and compares in constant time; a mismatch or a missing header answers the negotiate request with `401`, so a client without the password never reaches a hub method at all. A relay with no password configured admits every client and reports so through `RelayCapabilities`.

`EnterRoom` carries the room name in the clear — it is a label, not a secret. The relay normalises it (trimmed, case-folded, at most 64 characters, falling back to `public`) and holds it per connection, so it is named again after every connect and reconnect. Listings, join requests and directory notifications never cross rooms. Any client the password admitted may name any room.

An active host publishes its chosen display name, optional profile picture, and opaque session ID to the clients in the same room. A new session starts published and the host can hide it at any time. An annotator may submit a direct join request for a listed entry, but receives no credential and cannot send pointers until the host approves. Hidden, disconnected, pending, and full-capacity hosts are omitted from the listing. A host chooses a maximum annotator count when creating the session; the client default is two and the server maximum is sixteen. A published host stays available until it reaches capacity, hides/ends the session, or the session expires. The directory is the only way into a session; there is no other join path.
...
- `/version`: `ServerVersionResponse` — the constant product id `remote-annotate-relay` and the relay's build version without the commit metadata that Nerdbank.GitVersioning appends.

Both stay open on a protected relay, because a client has to be able to check that an address is a reachable relay before it can have been given the password for it. The settings-pane connection test uses both. `/health` establishes reachability, then `/version` establishes identity: the response must be `application/json`, must be small, and must carry the expected product id. Reachability alone is not enough, because `/health` is a common path that any unrelated host may answer. A host that fails the identity check is reported as "not a Remote Annotate server" and its address is not saved. The advertised version is shown next to the verified checkmark under the server address field.

## SignalR surface

Clients connect to `/hubs/pointer` with a persistent `clientInstanceId`, a process-scoped `applicationInstanceId`, an optional approval `displayName`, and — on a protected relay — the derived server-password key as a bearer token. The process-scoped identity prevents a running client from discovering or joining its own host session while still allowing separate client processes on the same machine. Every method below is behind the password; a client that does not hold it reaches none of them. Implemented client-to-server methods are:
...
Implemented server-to-client methods are `AnnotatorJoinRequested`, `AnnotatorJoinCancelled`, `SessionCredentialIssued`, `SessionApproved`, `HostDisplayChanged`, `PointerReceived`, `PointerDisplayed`, `AnnotationPaused`, `AnnotationColorAssigned`, `SessionEnded`, and `HostDirectoryChanged`. `HostDirectoryChanged` is broadcast whenever the directory could have changed — including when the relay collects an expired session, which no client asked for — and carries no payload: a client that cares re-reads the directory with `GetAvailableHosts`. It reaches the room the affected session was published in, which is not always the room of the connection that caused the change, because an approved annotator that changes rooms keeps its place in the session it was admitted to. A client that cannot act on a notification when it arrives, because a session of its own owns the listing, re-reads the directory when that session ends rather than dropping the notification.
...
`PointerEventMessage.Color` runs the other way: the annotator puts it there and the relay passes it through untouched, so a drawing looks the same on the host as it does under the hand that made it. It is `#RRGGBB` — exactly seven characters, upper-case hex, no alpha — and validation rejects anything else rather than coercing it. Opacity is deliberately not carried: how strongly each end draws is a local viewing preference, not the annotator's to dictate. An event that omits the colour is drawn in the default accent `#FF5C5C`. A gesture keeps the colour of the event that opened it for its whole life, so a colour changed mid-stroke cannot repaint what is already on screen.

Which colour an annotator may put there is the relay's to decide. `SetAnnotationColorPreference` registers the colour an approved annotator would like; the relay reallocates the whole session and answers each annotator whose colour moved with `AnnotationColorAssigned`. The caller is always answered, even when nothing moved, because it applied its own pick locally the moment the user made it and silence would leave the two ends disagreeing.

Allocation runs in join order, so an annotator already drawing is never moved to make room for one that just arrived. An annotator gets the colour it asked for — including a custom one — unless somebody ahead of it already holds it, in which case it is moved to a free colour from the fixed palette below. Only palette colours are handed out, so a colour nobody chose is still one the settings pane can show and name. Past seven annotators there is nothing distinct left to give and colours repeat, spread as evenly as the palette allows rather than piling onto one.

Because the session is reallocated from scratch after every join, departure and preference change, a displaced annotator returns to its own colour the moment the annotator holding it leaves; there is no separate restore step to drift out of step. The preference is session state and is never written to the annotator's settings, which keep whatever the user picked.

The palette is `#FF5C5C`, `#FFB900`, `#6CCB7F`, `#4FC3F7`, `#6C8CFF`, `#B388FF`, `#FF6FB5`. Its order is part of the contract, because the relay allocates from it in that order.
