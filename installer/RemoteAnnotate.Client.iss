...
[Setup]
AppId={{E8A5EAE5-05A6-47B5-AA18-6215C2A39C40}
AppName=Remote Annotate
AppVersion={#MyAppVersion}
AppPublisher=Remote Annotate
AppVerName=Remote Annotate {#MyAppVersion}
AppMutex=RemoteAnnotate.Client.Running
DefaultDirName={autopf}\Remote Annotate
DefaultGroupName=Remote Annotate
...
OutputBaseFilename=RemoteAnnotate.Client-{#MyAppVersion}-x64-Setup
...
UninstallDisplayIcon={app}\RemoteAnnotate.Client.exe
...
Name: "trustrelay"; Description: "Trust the Remote Annotate relay certificate"; GroupDescription: "HTTPS certificate:"
#endif
...
[Icons]
Name: "{autoprograms}\Remote Annotate"; Filename: "{app}\RemoteAnnotate.Client.exe"; WorkingDir: "{app}"
...
#endif
Filename: "{app}\RemoteAnnotate.Client.exe"; Description: "Launch Remote Annotate"; Flags: nowait postinstall skipifsilent
...
begin
  ...
  begin
    ...
    RegDeleteValue(HKEY_CURRENT_USER,
      'Software\Microsoft\Windows\CurrentVersion\Run', 'RemoteAnnotate');
    ...
    UserDataDir := ExpandConstant('{localappdata}\RemoteAnnotate');
    if DirExists(UserDataDir) and not UninstallSilent() then
    begin
      if MsgBox('Also remove your saved Remote Annotate settings and profile data?' + #13#10 + UserDataDir,
